...
from config import ASTEROID_BASE_SPEED, ASTEROID_VERTICES, ASTEROID_RADIUS_VARIATION, LEVEL


class Asteroid:
    def __init__(self, position, radius, split_level=0):
        ...
        self.destroy_score = random.randint(1000000, 5000000)

        speed = ASTEROID_BASE_SPEED * (10.0 / radius) * random.uniform(0.5, 1.5)
    ...
    def split(self):
        if LEVEL == 1 or self.split_level >= LEVEL - 1:
            return []

        new_asteroids = []
        new_radius = self.radius / 2
        for _ in range(2):
            new_asteroids.append(Asteroid(self.position, new_radius, self.split_level + 1))
